...
+ (NSString *)ageForBirthDayStyle:(NSString *)timeStamp ToDate:(NSDate *)nowDate{
    NSCalendar *calendar = [NSCalendar currentCalendar];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc]init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd"];
    //生日
    NSDate *birthday = [NSDate dateWithTimeIntervalSince1970:[timeStamp doubleValue]];
...
//        [birthdayStr appendFormat:@"%@岁",[self getAgeCapitalString:[date year]]];
    }
    if([date month] >0)
    {
        [birthdayStr appendFormat:@"%ld个月",(long)[date month]];
    }
    if([date day]>0){
...
+ (NSString *)getAgeCapitalString:(NSInteger)year{
    if (year > 99) {
        return @"0";
    }
    NSArray *arr =  [NSArray arrayWithObjects:@"十",@"一",@"二",@"三",@"四",@"五",@"六",@"七",@"八",@"九",nil];
    ...
    do {
        int remainder = 0;
        if (num < 10) {
            remainder = tmpYear%10;
        }else{
            remainder = tmpYear/num;
        }
        if (str.length) {
            str = [NSString stringWithFormat:@"%@%@",arr[remainder],str];
        }else{
            str = arr[remainder];
        }
        tmpYear -= remainder;
        ...
        if (num > year) {
            ...
            str = [NSString stringWithFormat:@"十%@",str];
        }
    } while (!finished);
    if ([str containsString:@"一十"]) {
        str = [str stringByReplacingOccurrencesOfString:@"一十" withString:@"十"];
    }
    if ([str containsString:@"十十"]) {
        str = [str stringByReplacingOccurrencesOfString:@"十十" withString:@"十"];
    }
    return str;
}

+ (NSString *)ageForBirthDayStyle:(NSString *)timeStamp{
    NSCalendar *calendar = [NSCalendar currentCalendar];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc]init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd"];
    //生日
...
//问题列表的时间格式
+ (NSString*)timeForQuestionStyle:(NSString *)timeStamp
{
    if (!timeStamp || ![timeStamp length]) {
        return nil;
    }
