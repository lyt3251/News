...
//    self.select = selectDate;//选择的日期
    ...
    for (int i = 0; i <= months; i++) {
        ...
        if (self.num == -1) {
            month = [self.today dayInThePreviousMonth];
        }else if (self.num == 1){
            month = [self.today dayInTheFollowingMonth];
        }else{
            month = [self.today dayInTheFollowingMonth:i];
        }
        
        self.nowShowDate = month;
        self.calendarDays = [[NSMutableArray alloc]init];
        [self calculateDaysInPreviousMonthWithDate:month andArray:self.calendarDays];
        [self calculateDaysInCurrentMonthWithDate:month andArray:self.calendarDays];
        if (type == CalendarShowTypeMultiple) {
            [self calculateDaysInFollowingMonthWithDate:month andArray:self.calendarDays];//计算下月份的天数
        }
        
//        [self calculateDaysIsWeekendandArray:calendarDays];
        
        [calendarMonth insertObject:self.calendarDays atIndex:i];
    }
