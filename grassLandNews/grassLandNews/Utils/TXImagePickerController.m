...
NSString *const TXImageSourceKey = @"txImageSourceKey";
NSString *const TXImageCreateDateKey = @"txImageCreateDateKey";
...
- (void)assetsPickerController:(GMImagePickerController *)picker didFinishPickingAssets:(NSArray *)assets
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(imagePickerController:didFinishPickingAssets:)]) {
            [_delegate imagePickerController:self didFinishPickingAssets:assets];
        }
    });
    if ([_delegate respondsToSelector:@selector(imagePickerController:didFinishPickingImagesAndDates:)]) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            NSMutableArray *dateImageArray = [NSMutableArray array];
...
//                            [dict setValue:result forKey:TXImageSourceKey];
                            NSData *data = UIImageJPEGRepresentation(result, 0.8);
                            [dict setValue:data forKey:TXImageSourceKey];

                            NSDate *date = obj.creationDate;
                            if (date) {
                                [dict setValue:date forKey:TXImageCreateDateKey];
                            }
                            [dateImageArray addObject:dict];
                        }
                    }];
                }
            }];
            dispatch_async(dispatch_get_main_queue(), ^{
                if (_delegate && [_delegate respondsToSelector:@selector(imagePickerController:didFinishPickingImagesAndDates:)]) {
                    [_delegate imagePickerController:self didFinishPickingImagesAndDates:dateImageArray];
                }
            });
        });
    }
    if ([_delegate respondsToSelector:@selector(imagePickerController:didFinishPickingImages:)]) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
...
- (void)imagePickerDidFinished:(JFImagePickerController *)picker
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(imagePickerController:didFinishPickingAssets:)]) {
            [_delegate imagePickerController:self didFinishPickingAssets:picker.assets];
        }
    });
    if ([_delegate respondsToSelector:@selector(imagePickerController:didFinishPickingImagesAndDates:)]) {
        NSMutableArray *dateImagesArr = [NSMutableArray array];
        //创建图片操作group
        dispatch_group_t imageGroup = dispatch_group_create();
        [picker.assets enumerateObjectsUsingBlock:^(ALAsset *asset, NSUInteger idx, BOOL *stop) {
            dispatch_group_enter(imageGroup);
            [[JFImageManager sharedManager] imageWithAsset:asset maxPixelSize:1080 resultHandler:^(CGImageRef imageRef) {
                NSMutableDictionary *dict = [NSMutableDictionary dictionary];
                UIImage *image = [UIImage imageWithCGImage:imageRef];
//                [dict setValue:image forKey:TXImageSourceKey];
                ...
                if ([date isKindOfClass:[NSDate class]]) {
                    ...
                    [dict setValue:imageDate forKey:TXImageCreateDateKey];
                }
                [dateImagesArr addObject:dict];
        ...
        dispatch_group_notify(imageGroup, dispatch_get_main_queue(), ^{
            if (_delegate && [_delegate respondsToSelector:@selector(imagePickerController:didFinishPickingImagesAndDates:)]) {
                [_delegate imagePickerController:self didFinishPickingImagesAndDates:dateImagesArr];
            }
        });
    }
    if ([_delegate respondsToSelector:@selector(imagePickerController:didFinishPickingImages:)]) {
