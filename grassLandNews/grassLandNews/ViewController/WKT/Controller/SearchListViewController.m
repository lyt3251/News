...
//    }
    ...
    [newsManager requestNewsListBySearchWords:nil page:self.recommandCurrentPage onCompleted:^(NSURLSessionDataTask *task, id responseObject, NSError *error) {
        ...
        {
            [self updateByDicList:dic[@"data"][@"data"]];
            NSNumber *totalPageNum = dic[@"data"][@"totalPage"];
...
-(void)searchNewsByKeyword
{
    ...
    @weakify(self);
    [TXProgressHUD showHUDAddedTo:self.view animated:YES];
    [newsManager requestNewsListByPage:self.currentPage nodeId:0 keyword:searchKey ids:nil clickdesc:0 aType:1  onCompleted:^(NSURLSessionDataTask *task, id responseObject, NSError *error) {
        @strongify(self);
        [TXProgressHUD hideHUDForView:self.view animated:YES];
        [self.tableView.header endRefreshing];
        NSDictionary *dic = (NSDictionary *)responseObject;
...
-(void)searchNextRecommandList
{
    ...
    [newsManager requestNewsListBySearchWords:nil page:self.recommandCurrentPage onCompleted:^(NSURLSessionDataTask *task, id responseObject, NSError *error) {
        ...
        {
            [self updateByNextDicList:dic[@"data"][@"data"]];
            self.recommandCurrentPage++;
            [self.tableView reloadData];
            self.tableView.footer.hidden  = self.recommandCurrentPage > self.recommandTotalPage?YES:NO;            
        }
